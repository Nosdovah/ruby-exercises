...
def correct_format?(word_list)
  ...
  word_list.none? { |letter| letter.scan(/\b[A-Z]+\b/)[0] and letter.upcase}
end

def valid_scores?(score_list, perfect_score)
  # use #one? to return true when only one value in the score_list (hash) is equal to the perfect_score (number)
  ##score_list.each { |key,value| puts "this is key #{key}, this is value #{value}"}
  score_list.one? { |scoreName,score| score == perfect_score}
end
